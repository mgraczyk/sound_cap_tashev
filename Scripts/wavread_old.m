function [y,Fs,bits,opt_ck] = wavreadfix(file,ext)
...
elseif (opt_ck.fmt.wFormatTag == 65534)
    % WAV format extensible has a separate field
    bits = opt_ck.fmt.wSamples;
else
    % for al other cases
    bits = opt_ck.fmt.nBitsPerSample;  % Unknown
end
...
if (total_bytes > nbytes),
   ...
   % read WAVE format Extensible fields if specifired and present
   if ((fmt.wFormatTag == 65534) && (total_bytes >= nbytes+2))
      % we have the wSamples ushort in the file:
      [wSamples,cnt]=fread(fid,1,'ushort');
      nbytes=nbytes+2;
      if (cnt~=1),
         msg = err_msg;
         return
      end
      fmt.wSamples = wSamples;
   end

   if ((fmt.wFormatTag == 65534) && (total_bytes >= nbytes+4))
      % we have the dwChannelMask ulong in the file:
      [dwChannelMask,cnt]=fread(fid,1,'ulong');
      nbytes=nbytes+4;
      if (cnt~=1),
         msg = err_msg;
         return
      end
      fmt.dwChannelMask = dwChannelMask;
   end
   
   if ((fmt.wFormatTag == 65534) && (total_bytes >= nbytes+16))
      % we have the SubFormat GUID in the file:
      [dwSubFormat,cnt]=fread(fid,4,'ulong');
      nbytes=nbytes+16;
      if (cnt~=4),
         msg = err_msg;
         return
      end
      fmt.dwSubFormat = dwSubFormat;
   end
...
case 65534
   % WAV format extended, can be PCM Format too:
   [dat,msg] = read_dat_pcm(datack,wavefmt,ext);
otherwise
...
elseif (BytesPerSample == 4),
    if (wavefmt.wFormatTag == 3)
        % 32-bit 16.8 float (type 1 - 32-bit)
        % 32-bit normalized floating point
        dtype = 'float';  
    elseif ((wavefmt.wFormatTag ~= 3) && (wavefmt.nBitsPerSample == 24))
        % 32-bit 24.0 float (type 1 - 24-bit)
        dtype = 'float';  
        BytesPerSample = 3;
    elseif (wavefmt.wFormatTag == 65534)
        % WAV format extensible, the SubFormat GUID determines the format
        if (wavefmt.dwSubFormat(1) == 1)
            % signed 32-bit integer
            dtype='bit32';
        elseif (wavefmt.dwSubFormat(1) == 3)
            % 32 bits float
            dtype='float';
        else
            % default case
...
elseif BytesPerSample==4,
    if wavefmt.wFormatTag ~= 3 &&  ~strcmp(dtype,'float'),    % Type 3 32-bit is already normalized
